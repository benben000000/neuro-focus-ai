...
import { SavedNoteSummary, subscribeToUserSummaries, deleteNoteSummary } from '../services/notes';
import { formatTime } from '../services/learning';
import type { UserProgress } from '../types';
import { MediaCarousel } from './MediaCarousel';
...
export function Profile() {
    ...
    const [savedSummaries, setSavedSummaries] = useState<SavedNoteSummary[]>([]);
    ...
    useEffect(() => {
        ...
        const unsubscribeSummaries = subscribeToUserSummaries(currentUser.uid, (summaries) => {
    ...
    const handleDeleteSummary = async (summaryId: string) => {
        if (!window.confirm('Are you sure you want to delete this summary? This cannot be undone.')) return;
        try {
            await deleteNoteSummary(summaryId);
            showSuccessMessage('Summary deleted successfully');
        } catch (e) {
            console.error(e);
            showErrorMessage('Failed to delete summary');
        }
    };
    ...
    const filteredPosts = activeFilter === 'saved' ? sortedSavedPosts :
        activeFilter === 'notes' ? [] : // Notes tab is handled separately
                ...
                {savedSummaries.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {savedSummaries.map(summary => (
                            <div key={summary.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow group">
                                <div className="p-5 space-y-4">
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${summary.subject === 'science_bio' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
                                                    summary.subject === 'history_lit' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                                                        summary.subject === 'math' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                                                            summary.subject === 'psych_soc' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
                                                                'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400'
                                                    }`}>
                                                    {summary.subject === 'science_bio' ? 'Science' :
                                                        summary.subject === 'history_lit' ? 'History' :
                                                            summary.subject === 'math' ? 'Math' :
                                                                summary.subject === 'psych_soc' ? 'Psych' : 'General'}
                                                </span>
                                                <span className="text-xs text-slate-400">
                                                    {new Date(summary.createdAt).toLocaleDateString()}
                                                </span>
                                            </div>
                                            <h3 className="font-bold text-slate-900 dark:text-white line-clamp-1" title={summary.title}>
                                                {summary.title}
                                            </h3>
                                        </div>
                                        <button
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                                            ...
                                            <span className="truncate max-w-[200px]">
                                                {summary.sourceFileNames.length} source file{summary.sourceFileNames.length !== 1 ? 's' : ''}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                <div className="px-5 py-3 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                                    <button
                                        onClick={() => navigate('/notes', { state: { loadSummaryId: summary.id } })}
                                    ...
                                    >
                                        Open Summary <ChevronRight size={14} />
                                    </button>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="py-12 text-center bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                        ...
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">No summaries yet</h3>
                        <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto mb-6">
                            Upload your notes to the Note Summarizer to generate AI-powered study guides.
                        </p>
                        <button
